(() => {
  ...
  function SearchScreen({ onPick }) {
    ...
    return h('main', { className: 'app-shell' },
      ...
      h(Card, { className: 'search-panel', variant: 'elevated', padding: 'md' }, h('div', { className: `search-field${query ? ' has-clear' : ''}` }, h('span', { className: 'search-icon', 'aria-hidden': true }, icon('search')), h(Input, { inputSize: 'lg', placeholder: T.placeholder, value: query, onChange: (event) => setQuery(event.target.value), 'aria-label': T.placeholder }), query && h(Button, { className: 'clear-search', variant: 'ghost', size: 'sm', type: 'button', onClick: () => setQuery(''), 'aria-label': 'Clear search' }, '×'))),
