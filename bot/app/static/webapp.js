(() => {
  ...
  function Editor({ anime, onBack, notify }) {
    ...
    const [options, setOptions] = useState({ score: true, genres: true, mark: true }); const [sending, setSending] = useState(false); const [postersLoading, setPostersLoading] = useState(true); const [tenraiUnavailable, setTenraiUnavailable] = useState(false);
    const displayTitle = titleLanguage === 'orig' ? anime.name : anime.title;
    useEffect(() => {
      let active = true;
      apiFetch(`/api/anime/${anime.id}/posters`).then((response) => response.ok ? response.json() : { posters: [], warnings: ['tenrai'] }).then((data) => {
        if (!active) return;
        setPosters((current) => mergePosters(current, data.posters));
        setTenraiUnavailable(data.warnings?.includes('tenrai') || false);
      }).catch(() => { if (active) setTenraiUnavailable(true); }).finally(() => { if (active) setPostersLoading(false); });
      return () => { active = false; };
    }, [anime.id]);
    useEffect(() => {
